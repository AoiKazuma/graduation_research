...
contract System {
    struct TransactionInfo { 
        /// @dev 取引情報を格納する構造体
        string sender;      /// @dev 発送者の名前
        string receiver;    /// @dev 受取人の名前
    ...
    mapping (uint => TransactionInfo) identInfoTransactionInfo; /// @dev 識別情報(取引番号)から取引情報を検索するマッピング
    ...
    function generate(uint _labelId, uint _transactionNo) public view returns (bytes32) {
        ...
        TransactionInfo memory _transactionInfo = identInfoTransactionInfo[_transactionNo]; /// @dev 取引番号から取引情報を検索して格納
    ...
    function setTransaction(string memory _sender, string memory _receiver) public returns (uint) {
        /// @dev 取引情報をセットする関数 -> 登録された取引番号を返す
        identInfoTransactionInfo[transactionCount] = TransactionInfo(_sender, _receiver, transactionCount); /// @dev 取引番号から取引情報を検索できるようにする
    ...
    function getTransactionInfo(uint _transactionNo) public view returns (string memory, string memory, uint) {
        /// @dev マッピングから識別情報(取引番号)をもとに取引情報を取得する関数
        return (identInfoTransactionInfo[_transactionNo].sender, 
                identInfoTransactionInfo[_transactionNo].receiver, 
                identInfoTransactionInfo[_transactionNo].transactionNo);
    } 
